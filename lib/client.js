...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      // ================= 小型内存 store =================
      // [ENI] 原生 debounce(替代依赖 timer 服务的 ctx.debounce)
      function debounce(fn, ms) {
        ...
        wrapped.cancel = () => { if (h !== null) { clearTimeout(h); h = null; } };
        return wrapped;
      }
      const listeners = new Set();
      // [ENI] 表面中和: 遍历 DOM, 大不透明面板清背景, 让壁纸整页透出
      let neutralized = [];
      function isSurfaceBg(cs) {
        const bg = cs.backgroundColor;
        if (typeof bg === "string" && bg !== "transparent" && bg !== "rgba(0, 0, 0, 0)" && bg.trim() !== "") return true;
        const bi = cs.backgroundImage;
        if (typeof bi === "string" && bi !== "none" && bi.trim() !== "") return true;
        return false;
      }
      // [ENI] 装饰性渐变遮罩: 纯渐变背景 + 不吃指针事件(pointer-events:none) = 滚动淡出/压暗蒙层,
      // 不论尺寸都清掉(短条状的"会话栏底部灰色渐变"不满足 bigEnough, 否则会漏网)。含 url() 的真实底图不动。
      function isDecorativeGradient(cs) {
        const bi = cs.backgroundImage;
        if (typeof bi !== "string" || bi === "none" || bi.indexOf("gradient") === -1) return false;
        if (bi.indexOf("url(") !== -1) return false;
        return cs.pointerEvents === "none";
      }
      // [ENI] 滚动淡出常用 mask-image 渐变裁切内容, 清掉让整段区域(及其后壁纸)完整透出。
      function hasMask(cs) {
        const m = cs.maskImage;
        const wm = cs.webkitMaskImage;
        return (typeof m === "string" && m !== "none" && m.trim() !== "")
          || (typeof wm === "string" && wm !== "none" && wm.trim() !== "");
      }
      function neutralizeSurfaces() {
        ...
        const walk = (el) => {
          for (const child of el.children) {
            ...
            seen.add(child);
            // 本插件自己的视频层/压暗层不参与中和(否则压暗层的背景会被清成透明)。
            if (child.getAttribute && child.getAttribute("data-wp-own") !== null) continue;
            let cs = null;
            try { cs = getComputedStyle(child); } catch (e) { continue; }
            if (cs === null) continue;
            const z = cs.zIndex;
            const zNum = z !== "auto" ? parseFloat(z) : 0;
            if (zNum >= 100) continue; // 弹窗/模态(高 z)保持不透明
            let r = null;
            try { r = child.getBoundingClientRect(); } catch (e) { continue; }
            if (r === null) continue;
            const bigEnough = (r.width >= vw * 0.03 && r.height >= vh * 0.04) || (r.width >= vw * 0.5 && r.height >= 6);
            let touchedBg = false;
            let touchedMask = false;
            if (bigEnough && isSurfaceBg(cs)) {
              child.style.backgroundColor = "transparent";
              if (typeof cs.backgroundImage === "string" && cs.backgroundImage !== "none") {
                child.style.backgroundImage = "none";
              }
              touchedBg = true;
            } else if (isDecorativeGradient(cs)) {
              child.style.backgroundImage = "none";
              touchedBg = true;
            }
            if (hasMask(cs)) {
            ...
            // 仅背景中和才打标记(触发 !important 清背景规则); 纯遮罩元素只清 mask, 不动其背景。
            if (touchedBg) child.setAttribute("data-wp-neutralized", "");
            if (touchedBg || touchedMask) neutralized.push(child);
            walk(child);
          }
        };
        walk(document.body);
      ...
      function restoreSurfaces() {
        for (const el of neutralized) {
          ...
          if (el) el.removeAttribute("data-wp-neutralized");
        }
        neutralized = [];
      }
      ...
      styleEl.textContent = `
        ...
        [data-wp-neutralized] { background: transparent !important; background-image: none !important; }
      ...
      // [ENI] 视频背景层(独立 <video> 元素, 与图片层共存, 淡入淡出)
      let videoEl = null;
      // [ENI] 视频专用压暗层: 图片的压暗渐变内嵌在 html::before/::after 里, 视频层不经过那条路,
      // 需单独盖一层 rgba(0,0,0,var(--wp-scrim)); 否则切到视频壁纸时"壁纸压暗"渐变会整个消失。
      let videoScrimEl = null;
      function ensureVideoEl() {
        if (videoEl === null && typeof document !== "undefined" && document.body !== null) {
          videoEl = document.createElement("video");
          videoEl.muted = true;
          videoEl.loop = true;
          videoEl.playsInline = true;
          videoEl.autoplay = true;
          videoEl.setAttribute("data-wp-own", "");
          videoEl.style.cssText = "position:fixed;inset:0;width:100%;height:100%;object-fit:cover;z-index:-1;pointer-events:none;opacity:0;transition:opacity var(--wp-dur,1200ms) ease;background:transparent;filter:var(--dswp-blur,none);";
          document.body.appendChild(videoEl);
          videoEl.addEventListener("loadeddata", () => {
            try { videoEl.muted = true; void videoEl.play().catch(() => {}); } catch (e) {}
          });
          // 压暗层紧跟视频之后插入(同 z-index:-1, 靠 DOM 次序压在视频之上、UI 之下); 深浅随 --wp-scrim 实时联动。
          videoScrimEl = document.createElement("div");
          videoScrimEl.setAttribute("data-wp-own", "");
          videoScrimEl.style.cssText = "position:fixed;inset:0;z-index:-1;pointer-events:none;opacity:0;transition:opacity var(--wp-dur,1200ms) ease;background:rgba(0,0,0,var(--wp-scrim,0));";
      ...
      function setVideo(url) {
        ...
        try {
          ...
          v.style.opacity = "1";
          if (videoScrimEl !== null) videoScrimEl.style.opacity = "1";
          void v.play().catch(() => {});
        } catch (e) {}
      }
      function clearVideo() {
        ...
        if (videoScrimEl !== null) videoScrimEl.style.opacity = "0";
      }

      let currentUrl = null;
      let fadeToken = 0;
      let surfObserver = null;
      let surfRaf = null;
      function rescanSurfacesSoon() {
        if (surfRaf !== null) return;
        surfRaf = requestAnimationFrame(() => {
          surfRaf = null;
          if (state.enabled) neutralizeSurfaces();
        });
      }
      function ensureSurfObserver() {
        if (surfObserver !== null || typeof MutationObserver === "undefined") return;
        surfObserver = new MutationObserver(() => rescanSurfacesSoon());
        surfObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["class", "style"] });
      }
      async function applyWallpaper(entry, fadeIn) {
        const url = entry ? entry.url : null;
        const kind = entry && entry.kind === "video" ? "video" : "photo";
      ...
      const rotationHandle = setInterval(tick, 1000);
      ctx.effect(() => () => clearInterval(rotationHandle), "wallpaper-rotator: rotation");
      ...
      ctx.effect(() => () => {
        ...
        restoreSurfaces();
        if (surfObserver !== null) { try { surfObserver.disconnect(); } catch (e) {} surfObserver = null; }
        if (surfRaf !== null) { cancelAnimationFrame(surfRaf); surfRaf = null; }
        try { clearVideo(); } catch (e) {}
        if (videoEl !== null) { try { videoEl.remove(); } catch (e) {} videoEl = null; }
        if (videoScrimEl !== null) { try { videoScrimEl.remove(); } catch (e) {} videoScrimEl = null; }
