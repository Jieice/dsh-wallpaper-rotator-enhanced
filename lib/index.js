...
async function scanFolder(folder) {
  ...
  for (const entry of entries) {
    ...
    if (!IMAGE_RE.test(entry.name) && !VIDEO_RE.test(entry.name)) continue;
...
async function handleImage(req, res, rawName) {
  ...
  try {
    ...
    if (st.size > (isVideo ? MAX_VIDEO_BYTES : MAX_BYTES)) {
      res.writeHead(413);
      res.end();
      return;
    }
    ...
    if (isVideo) {
      // 视频: Range 请求支持(浏览器 video 元素 seek 必需)
      const size = st.size;
      ...
      if (range) {
        try {
          ...
          if (m !== null) {
            ...
            const rstream = createReadStream(filePath, { start, end });
            rstream.on("error", () => { try { res.destroy(); } catch (e) {} });
            rstream.pipe(res);
            return;
          }
        } catch (e) { /* fallthrough to 200 */ }
      }
      res.writeHead(200, {
        "content-type": mime,
